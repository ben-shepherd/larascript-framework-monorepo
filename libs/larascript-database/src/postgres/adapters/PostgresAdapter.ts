 ...
 */
export class PostgresAdapter extends BaseDatabaseAdapter<IPostgresConfig> implements IPostgresAdapter {
    ...
    async connectDefault(): Promise<void> {
        ...
        const { username: user, password, host, port, database} = ParsePostgresConnectionUrl.parse(this.getConfig().uri);
    ...
    getPgClientWithDatabase(database: string = 'postgres'): pg.Client {
        const { username: user, password, host, port } = ParsePostgresConnectionUrl.parse(this.getConfig().uri);
