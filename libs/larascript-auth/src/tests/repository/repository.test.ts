...
describe("Repository", () => {
    ...
    describe("base", () => {
        test("should update a item", async () => {
            ...
            const user = (await userRepository.getRecords())[0] as unknown as IUserModel;
        ...
        test("should set data", async () => {
            ...
            expect((await userRepository.getRecords())[0].getEmail()).toBe('test2@test.com');
        });
        ...
        describe("user", () => {
            ...
            test("should return a user by email", async () => {
                const foundUser = await userRepository.findByEmail(user.getEmail() as string);
            ...
            test("should throw an error if user is not found", async () => {
                expect(userRepository.findByIdOrFail('not-found')).rejects.toThrow('User not found');
            });
        })
        ...
        describe("apiToken", () => {
            ...
            test("should return null if api token is not found", async () => {
                const foundApiToken = await apiTokenRepository.findOneActiveToken('invalid');
            ...
            test("should revoke all api tokens for a user", async () => {
                ...
                const foundApiToken = await apiTokenRepository.findOneActiveToken(apiToken.getToken());
                const foundApiToken2 = await apiTokenRepository.findOneActiveToken(apiToken2.getToken());
