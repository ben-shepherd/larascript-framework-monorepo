...
import { IBelongsToOptions, IdGeneratorFn, IEloquent, IHasManyOptions, IRelationship } from '@/eloquent';
...
import { GetAttributesOptions, IModel, IModelAttributes, IModelEvents, IModelFactory, IModelFactoryConstructor, IModelLifeCycleEvent, ModelConstructor, ModelWithAttributes, TModelScope } from '..';
...
export default abstract class Model<Attributes extends IModelAttributes> implements IModel<Attributes> {
     ...
     */
    public connection: string = DB.getInstance().databaseService().getDefaultConnectionName();
    ...
    static query<Model extends IModel>(): IEloquent<Model> {
        return DB.getInstance().queryBuilder(this as unknown as ModelConstructor<Model>) as IEloquent<Model>
    }
    ...
    protected async getRelationshipData<K extends keyof Attributes = keyof Attributes>(key: K, relationship?: IRelationship): Promise<Attributes[K] | null> {
        ...
        const resolver = DB.getInstance().databaseService().getAdapter(this.connection).getRelationshipResolver()
    ...
    private queryBuilder(): IEloquent<IModel> {
        return DB.getInstance().queryBuilder(this.constructor as unknown as ModelConstructor<IModel>, this.connection);
    }
    ...
    encryptAttributes(attributes: Attributes | null): Attributes | null {
        ...
        this.encrypted.forEach(key => {
            ...
            if (typeof attributesCopy?.[key] !== 'undefined' && attributesCopy?.[key] !== null) {
                try {
                    (attributesCopy as object)[key] = DB.getInstance().cryptoService().encrypt((attributesCopy as object)[key]);
                }
                catch (e) {
                    console.error(e)
                }
    ...
    decryptAttributes(attributes: Attributes | null): Attributes | null {
        ...
        this.encrypted.forEach(key => {
            ...
            if (['object', 'array'].includes(this.casts?.[key] as string) && typeof attributesCopy?.[key] === 'string') {
                try {
                    attributesCopy[key] = JSON.parse(
                        DB.getInstance().cryptoService().decrypt(attributesCopy?.[key] as string)
                    )
                    return;
                }
                // eslint-disable-next-line no-unused-vars
            ...
            if (typeof attributesCopy?.[key] !== 'undefined' && attributesCopy?.[key] !== null) {
                try {
                    (attributesCopy as object)[key] = DB.getInstance().cryptoService().decrypt((attributesCopy as object)[key]);
                }
    ...
    protected async emit(event: IModelLifeCycleEvent, ...args: any[]): Promise<void> {
        if (typeof this.events?.[event] === 'function') {
            await DB.getInstance().eventsService().dispatch(new this.events[event](...args));
        }
    }
