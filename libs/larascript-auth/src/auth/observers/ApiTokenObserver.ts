...
export default class ApiTokenObserver extends Observer<IApiTokenObserverData> {  
    ...
    async addGroupScopes(data: IApiTokenObserverData): Promise<IApiTokenObserverData> {
        const user = await AuthEnvironment.getInstance().authService.getUserRepository().findByIdOrFail(data.userId)
        ...
        for(const userGroup of userGroups) {
            const scopes = AuthEnvironment.getInstance().aclService.getGroupScopes(userGroup)
