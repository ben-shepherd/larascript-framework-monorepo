...
describe("AuthService", () => {
    ...
    describe("boot", () => {
        ...
        test("should fail for invalid adapter name", () => {
            expect(() => authService.getAdapter('invalid')).toThrow(new Error('Adapter invalid not found'));
        });
    });
    ...
    describe("getJwt", () => {
        test("should return JWT adapter with correct config", async () => {
            ...
            expect(jwtAdapter.getConfig().options.factory.apiToken).toBe(TestApiTokenFactory);
            expect(jwtAdapter.getConfig().options.repository.user).toBe(InMemoryUserRepository);
            expect(jwtAdapter.getConfig().options.repository.apiToken).toBe(InMemoryApiTokenRepository);
        });
