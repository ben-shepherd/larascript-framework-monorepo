...
import { IDatabaseConfig } from "../interfaces/config.t";
...
class Database implements IDatabaseService, RequiresDependency {
    ...
    async connectDefault(): Promise<void> {
        if (!this.connectionsConfig[this.getDefaultConnectionName()]) {
            throw new DatabaseConnectionException('Connection not found: ' + this.getDefaultConnectionName())
        }
     ...
     * @private
     */
    async connectAdapter(connectionName: string = this.getDefaultConnectionName()): Promise<void> {
        
        if (!this.adapters[connectionName]) {
            throw new DatabaseConnectionException('Adapter not found: ' + connectionName)
        }
     ...
     * @returns 
     */
    isRegisteredAdapter(adapter: IDatabaseAdapterConstructor<IDatabaseAdapter>, connectionName: string = this.getDefaultConnectionName()): boolean {
        const adapterName = DatabaseAdapter.getName(adapter)

        for(const connectionName of Object.keys(this.adapters)) { 
            if(DatabaseAdapter.getName(this.getAdapterConstructor(connectionName)) === adapterName) {
                return true
            }
        }
     ...
     * @throws {Error} If the connection or adapter is not registered.
     */
    getAdapter<Adapter extends IDatabaseAdapter = IDatabaseAdapter>(connectionName: string = this.getDefaultConnectionName()): Adapter {
        if(!this.adapters[connectionName]) {
    ...
    getAllAdapterConstructors(): TClassConstructor<IDatabaseAdapter>[] {
        ...
        return Object.values(adaptersObject) as TClassConstructor<IDatabaseAdapter>[]
    }
