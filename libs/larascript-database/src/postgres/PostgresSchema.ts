...
class PostgresSchema extends BaseSchema<PostgresAdapter> {
    ...
    async createDatabase(name: string): Promise<void> {
        ...
        try {
            ...
            DB.getInstance().logger()?.error(`Failed to create database ${name}: ${(err as Error).message}`);
            throw err;
        }
        finally {
            await client.end()
    ...
    async databaseExists(name: string): Promise<boolean> {
        ...
        try {
            ...
            DB.getInstance().logger()?.error(`Failed to check if database ${name} exists: ${(err as Error).message}`);
            throw err;
        }
        finally {
            await client.end();
    ...
    async terminateConnections(database: string, client?: pg.Client): Promise<void> {
        ...
        // Terminate any remaining connections to the target database
            ...
            DB.getInstance().logger()?.error(`Failed to terminate connections to database ${database}: ${(err as Error).message}`);
            throw err;
        }
        finally {
            if(endConnection) {
    ...
    async createTable(tableName: string, attributes: ModelAttributes, optons?: QueryInterfaceCreateTableOptions): Promise<void> {
        try {
            ...
            DB.getInstance().logger()?.error(`Failed to create table ${tableName}: ${(err as Error).message}`);
            throw err;
        }
    }
    ...
    async dropTable(tableName: string, options?: QueryInterfaceDropTableOptions): Promise<void> {
        try {
            ...
            DB.getInstance().logger()?.error(`Failed to drop table ${tableName}: ${(err as Error).message}`);
            throw err;
        }
    }
    ...
    async alterTable(tableName: IAlterTableOptions['tableName'], options: Omit<IAlterTableOptions, 'tableName'>): Promise<void> {
        try {
            ...
            DB.getInstance().logger()?.error(`Failed to alter table ${tableName}: ${(err as Error).message}`);
            throw err;
        }
    }
    ...
    async tableExists(tableName: string): Promise<boolean> {
        try {
            ...
            DB.getInstance().logger()?.error(`Failed to check if table ${tableName} exists: ${(err as Error).message}`);
            throw err;
        }
    }
    ...
    async dropAllTables(): Promise<void> {
        try {
            ...
            DB.getInstance().logger()?.error(`Failed to drop all tables: ${(err as Error).message}`);
            throw err;
        }
    }
